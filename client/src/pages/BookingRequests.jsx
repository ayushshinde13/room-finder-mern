...
import { Calendar, MapPin, User, Mail, ShieldAlert, Sparkles, CheckCircle2, AlertCircle, XCircle, Trash2, Check, X } from "lucide-react";
  ...
  const updateBookingStatus = async (bookingId, status) => {
    try {
      ...
      if (response.ok) {
        ...
        toast.error(`Failed to ${status.toLowerCase()} booking: ${data.message}`);
      }
    } catch (error) {
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      <div className="mb-8">
        ...
        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
          Review, approve, or reject rental requests submitted by prospective tenants.
        </p>
      </div>
      ...
      {bookings.length === 0 ? (
        ...
        >
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-2 max-w-sm mx-auto">
            When renters submit requests to book your listed properties, they will show up here.
          </p>
        </motion.div>
      ) : (
              ...
              <div className="md:w-2/3 flex flex-col justify-between">
                <div className="space-y-2">
                  ...
                  <div className="mt-4 p-4 bg-slate-50/50 dark:bg-slate-900/40 rounded-2xl border border-slate-200/20 dark:border-slate-800/20 space-y-2">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block mb-1">Prospective Renter</span>
                ...
                <div className="mt-6 pt-4 border-t border-slate-200/50 dark:border-slate-800/50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    {booking.status === 'PENDING' && (
                      <div className="flex gap-2">
                        <button
                          ...
                          className="px-4 py-2 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white rounded-xl text-xs font-bold transition-all shadow-md shadow-emerald-500/5 flex items-center gap-1.5"
                        >
                          <Check size={14} />
                          <span>Approve</span>
                        </button>
                        <button
                          ...
                          className="px-4 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5"
                        >
                          <X size={14} />
                          <span>Reject</span>
                        </button>
                    ...
                    {booking.status === 'APPROVED' && (
                      <div className="inline-flex items-center gap-1 text-xs text-emerald-500 font-bold px-2.5 py-1 rounded bg-emerald-500/10 border border-emerald-500/20">
